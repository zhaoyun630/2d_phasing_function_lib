% ab initio phasing algorithm for 2D crystal
% algorithm used: Charge flipping + HIO + ER
% First created by Yun Zhao on Oct 28th.
% Last editted by Yun Zhao on Oct 28th.
...
phase_cc = phase_cc_numerator/phase_cc_denominator;
  ...
  %%%%%%%%%%%% shrink wrap %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   gprime_kxy_abs = abs(gprime_kxy);
   Imax = max(max(max(gprime_kxy_abs)));
   Icut = Imax*0.01;
   Ilevel = gprime_kxy_abs- Icut;
   Mat_shrink = sign(sign(Ilevel)+1);
   support = Mat_shrink.*support;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
