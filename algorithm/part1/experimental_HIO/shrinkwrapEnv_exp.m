% ######################################################################
%  Shrinkwrap with a hole constraint in middle and corner of unit cell
% ######################################################################
%% Version
% First Created by Yun Zhao on Feb 13rd, 2015
% Last editted by Yun Zhao on Mar 29th, 2016
% 
% Compared with shrinkwrapEnv.m, this version doesn't take real part of
% Fourier Transform
% *************************************************************************
%% Notation:
...
figureOfMerit = zeros(iterComb*(iterHIO+iterER),3);
...
objEstimatePrime = ifftn(FComplexCorneredEstimatePrime); 
objEstimate = real(objEstimatePrime.* support1);  % apply known support
...
for i=1:iterComb
    ...
    for j = 1:iterHIO
        iterIndex = (i-1)*(iterHIO+iterER) + j;
        ...
        objEstimatePrime = real(ifftn(FComplexCorneredEstimated));
        ...
        objEstimate = real(objEstimate1) + real(objOutsideSupport); % new object estimation for next iteration.
...
%         figureOfMerit(iterIndex,3) = getRFactor(FModulusCorneredModel,FModulusCorneredEstimate,HKLmask);
    ...
    for t = 1:iterER
        iterIndex = (i-1)*(iterHIO+iterER)+iterHIO + t;
        FComplexCorneredEstimatePrime = fftn(objEstimate);
        FModulusCorneredEstimate = abs(FComplexCorneredEstimatePrime);
        ...
        % Impose support from real space
        objEstimate = real(objEstimatePrime.*support1);
...
% Show reconstruction
show3DObjv2(objFinal,0.95,1,1);

% Show model
show3DObjv2(model,0.95,1,1);
