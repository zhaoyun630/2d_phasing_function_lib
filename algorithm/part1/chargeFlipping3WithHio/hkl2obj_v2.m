...
% Compared with inital version.
% This version take the size of triple cell as 3*s_c - 2, where it 
% is 3*s_c in version 1.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 if isempty(hkl_file_name)
     hkl_file_name = '3rdx_full_3A.hkl';
 end
...
%  % Triple in c direction
 obj_triple_c = zeros(s_a,s_b,3*s_c-2);
 support_c = zeros(s_a,s_b,3*s_c-2);
 obj_triple_c(:,:,s_c:2*s_c-1) = unit_cell_abs;
 support_c(:,:,s_c:2*s_c-1) = ones(s_a,s_b,s_c);
 save('obj_abs_triple_3A.mat','obj_triple_c');
