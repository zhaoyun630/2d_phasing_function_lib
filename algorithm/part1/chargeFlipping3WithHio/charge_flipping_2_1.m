...
% Compared with version charge_flipping_2_2, this piece take obj file as
% input

F_mask_centered = importdata('rec_lattice_mask.mat');
F_mask_uncentered = ifftshift(F_mask_centered);

obj=importdata('unit_cell_real_ifftshift.mat');   
F_modulus_shifted_observed=abs(fftn(obj)).*F_mask_uncentered;     %Discard phases of transform of test object. We will try to find these using the Fienup HIO algorithm.
% after fftshift, the center is F(000).
%  F_modulus_centered_observed = fftshift(modulus);

% F_complex_centered_observed = importdata('rec_lattice.mat');
...
for iter = 1: iter_num_total
    ...
    F_modulus = abs(F_comp_1).*F_mask_uncentered;
...
%     end
    ...
    obj_k = ifftn(F_comp_2);
    
end

save('obj_obj_600iter.mat','obj_k');
save('r_factor_obj_600iter.mat','R_factor');


R_factor = importdata('r_factor_600iter_1A.mat');
...
saveas(plt1,'r_factor_iteration_obj_600.tif');
