% Given prior information within a radius.
...
phase_cc_numerator=sum(sum(sum(phase_w)))-sum(sum(sum(phase_w(a_r,b_r,c_r))));
...
rms_0 = sum(sum(sum(gk)));				%first iteration to get rms1 (root mean square of support area after first iteration). 
gkxy = gprime_kxy.*support;			%rms1 is used in subsequent iterations as normalization factor. 
gkos = (abs(gprime_kxy.*support2)).^2;
rms_1 = sum(sum(sum(gkos)));
rms = sqrt(rms_1/rms_0);
% imshow(abs(gkxy),[]);
...
for w=1:4
 for j = 1:20                      % Do HiO.   change to desired value
  ...
  % Gprime_kuv = modulus.*exp(1i*angle(Gkuv));
   ...
   rms_0 = sum(sum(sum(abs(gprime_kxy).^2)));
   gk = (abs(gprime_kxy.*support2)).^2;    % error calcn.  DM program does not have square.
   rms = sqrt(sum(sum(sum(gk)))/rms_0);  % i updated this line by adding sqrt
   phase_w = modulus.^2.*cos(angle(Gkuv)-phase_true);
   phase_cc_numerator=sum(sum(sum(phase_w)))-sum(sum(sum(phase_w(a_r,b_r,c_r))));
