function obj = hkl2obj_v2(hklFile, densityShift)
...
for i=1:L_M
    ...
    Rec_lattice(h1,k1,l1) = F_comp(i);
    Rec_lattice(h2,k2,l2) = F_comp_c(i);
    Rec_lattice_mask(h1,k1,l1) = 1;
    Rec_lattice_mask(h2,k2,l2) = 1;
end

% Keep an eye on the value of F000, it may be zero or some other constant.
Rec_lattice(h_max+1,k_max+1,l_max+1)=F_000;

% Structure factor list. F_000 is in the center of 3D array.
% save('rec_lattice_3rdx.mat','Rec_lattice');
% save('rec_lattice_mask_3rdx.mat','Rec_lattice_mask');
...
%  unit_cell_real = fftshift(real(unit_cell_complex)); % I think I shouldn't use fftshift.
 unit_cell_real = real(unit_cell_complex);
